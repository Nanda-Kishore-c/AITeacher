...
def validate_and_save_audio(wav, sr, outwav_path):
    ...
    if not outwav_path or not outwav_path.strip():
        return False, "Output path is empty or invalid."
    ...
    try:
        ...
        dirname = os.path.dirname(outwav_path)
        if dirname:  # Only if there's a directory part
